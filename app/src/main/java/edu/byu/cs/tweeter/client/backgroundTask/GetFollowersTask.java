...
public class GetFollowersTask implements Runnable {
    ...
    public GetFollowersTask(AuthToken authToken,
                            User targetUser,
                            int limit,
                            User lastFollower,
    ...
    private Pair<List<User>, Boolean> getFollowers() throws IOException, TweeterRemoteException {
        String alias = null;
        if (this.lastFollower != null) {
            alias = this.lastFollower.getAlias();
        }
        FollowersRequest followersRequest = new FollowersRequest(this.authToken, this.targetUser.getAlias(), this.limit, alias);
